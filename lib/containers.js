...
module.exports = {
    getLogs: (req, res, next) => {
        ...
        //reading logs directly from docker is only supported on argo-managed barges
        if (req.helmit.bargeInfo.argoManaged) {
            ...
            if (req.helmit.nodes.length) {
                ...
                req.helmit.nodes.filter(ele => ele.phase == "running").forEach(node => {
            ...
            res.status(400).json({
                error: true,
                message: `not supported on barge: ${req.helmit.bargeInfo.barge}; use /v2/harbor/logs instead`
            });
        }
    }
};
